...
{
	name = "[CSGO] Custom Chat",
	author = "Retro | Edited: somebody.",
...
{	
	DB_Load();
	RegAdminCmd("sm_chatcolors", Command_Colors, ADMFLAG_RESERVATION);
	RegAdminCmd("sm_namecolors", Command_NameColors, ADMFLAG_RESERVATION);
	RegAdminCmd("sm_settag", Command_SetTag, ADMFLAG_RESERVATION);
...
public Action Command_SetTag(int client, int args)
{
	...
	if(args == 1)
	{
		...
		if(StrEqual("off", arg, false))
		{
			...
			PrintToChat(client, " \x06[CustomChat] \x0BTörölted az egyéni chat címkédet.");
	...
	Format(msg, sizeof(msg), " \x06[CustomChat] \x0BBeállítottad az egyéni chat címkédet: %s", arg_string);
...
void DB_Load()
{
	Database.Connect(DB_Connect, "customchat");
}
...
public void OnMapStart()
{
	...
	menu.SetTitle("Válassz színt az üzenetednek:");
	for (int i = 0; i < sizeof(g_sColorNames); i++)
	{
		char info[16];
	...
	menu2.SetTitle("Válassz színt a nevednek:");
	for (int i = 0; i < sizeof(g_sColorNames); i++)
	{
		char info[16];
		IntToString(i, info, sizeof(info));
...
public int MenuHandler_ChatColor(Menu menu, MenuAction action, int client, int choice)
{
	if (action != MenuAction_Select)
		return;
	PrintToChat(client, " \x06[CustomChat] \x0BBeállítottad a chat üzeneteid színét: %s%s\x01.", g_sColorCodes[choice], g_sColorNames[choice]);
...
public int MenuHandler_NameColor(Menu menu, MenuAction action, int client, int choice)
{
	if (action != MenuAction_Select)
		return;
	PrintToChat(client, " \x06[CustomChat] \x0BBeállítottad a neved színét: %s%s\x01.", g_sColorCodes[choice], g_sColorNames[choice]);
...
public Action CP_OnChatMessage(int& author, ArrayList recipients, char[] flagstring, char[] name, char[] message, bool& processcolors, bool& removecolors)
{
	...
	if (n)
	{
		...
		{
			Format(name, 255, "%s%s", g_sColorCodes[g_iPlayerInfo[author][Name]], name);
			changed = true;
			needspace = true;
		}
	}

	Format(name, 255, "%s%s%s", needspace ? " " : "", ctag, name);
