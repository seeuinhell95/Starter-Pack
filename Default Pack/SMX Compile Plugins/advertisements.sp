...
#include "advertisements/globals.sp"
#include "advertisements/client.sp"

#define LoopClients(%1) for(int %1 = 1;%1 <= MaxClients;%1++) if(IsValidClient(%1))

#include "advertisements/misc.sp"

#pragma newdecls required

public Plugin myinfo =
{
	name = "[CSGO] Server Advertisements",
	author = "ESK0 | Edited: somebody.",
	description = "Server Advertisements",
	version = "1.0",
...
public void OnPluginStart()
{
	...
	RegAdminCmd("sm_sar", Command_sa3r, ADMFLAG_ROOT, "Message reload");
	RegAdminCmd("sm_sa3r", Command_sa3r, ADMFLAG_ROOT, "Message reload");

	RegAdminCmd("sm_sa3lang", Command_ChangeLanguage, ADMFLAG_ROOT, "Message language");

	BuildPath(Path_SM, sConfigPath, sizeof(sConfigPath), "configs/advertisements.cfg");
	...
	g_hSA3CustomLanguage = RegClientCookie("sa_customlanguage", "Custom langauge for SA3", CookieAccess_Private);
...
public void LoadConfig()
{
	...
	KeyValues kvConfig = new KeyValues("ServerAdvertisement");
	if(FileExists(sConfigPath) == false)
	{
		SetFailState("%s Unable to find advertisements.cfg in %s", SA3, sConfigPath);
		return;
	}
	kvConfig.ImportFromFile(sConfigPath);
	if(kvConfig.JumpToKey("Settings"))
	{
		kvConfig.GetString("ServerName", sServerName, sizeof(sServerName), "[Advertisements]");
...
{
	aMessagesList.Clear();
	KeyValues kvMessages = new KeyValues("ServerAdvertisement");
	if(FileExists(sConfigPath) == false)
	{
		SetFailState("%s Unable to find advertisements.cfg in %s", SA3, sConfigPath);
		return;
