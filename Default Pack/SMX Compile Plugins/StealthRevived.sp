...
{
	name = "[CSGO] Stealth Revived", 
	author = "SM9(); | Edited: somebody.", 
	description = "Stealth Revived", 
	version = "1.0", 
...
{
	g_cvCustomStatus = CreateConVar("sm_stealthrevived_status", "1", "Should the plugin rewrite status?", _, true, 0.0, true, 1.0);
	g_cvCustomStatus.AddChangeHook(OnCvarChanged);

	g_cvSetTransmit = CreateConVar("sm_stealthrevived_hidecheats", "1", "Should the plugin prevent cheats with 'spectator list' working? (This option may cause performance issues on some servers)", _, true, 0.0, true, 1.0);
	g_cvSetTransmit.AddChangeHook(OnCvarChanged);

	AutoExecConfig(true, "StealthRevived");
