...
LastRequest_OnPluginStart()
{
	...
	gH_Cvar_LR_GunToss_On = CreateConVar("sm_hosties_lr_gt_enable", "0", "Enable LR Gun Toss: 0 - disable, 1 - enable", 0, true, 0.0, true, 1.0);
	gShadow_LR_GunToss_On = true;
	gH_Cvar_LR_ChickenFight_On = CreateConVar("sm_hosties_lr_cf_enable", "0", "Enable LR Chicken Fight: 0 - disable, 1 - enable", 0, true, 0.0, true, 1.0);
	...
	gH_Cvar_LR_Rebel_On = CreateConVar("sm_hosties_lr_rebel_on", "0", "Enables the LR Rebel: 0 - disable, 1 - enable", 0, true, 0.0, true, 1.0);
	...
	gH_Cvar_LR_JumpContest_On = CreateConVar("sm_hosties_lr_jumpcontest_on", "0", "Enables the LR Jumping Contest: 0 - disable, 1 - enable", 0, true, 0.0, true, 1.0);	
	...
	gH_Cvar_LR_ChickenFight_Slay = CreateConVar("sm_hosties_lr_cf_slay", "0", "Slay the loser of a Chicken Fight instantly? 0 - disable, 1 - enable", 0, true, 0.0, true, 1.0);
	...
	gH_Cvar_LR_Dodgeball_Gravity = CreateConVar("sm_hosties_lr_db_gravity", "0.4", "What gravity multiplier the dodgeball contestants will get: <1.0 - less/lower, >1.0 - more/higher", 0, true, 0.1, true, 2.0);
	...
	gH_Cvar_LR_GunToss_StartMode = CreateConVar("sm_hosties_lr_gt_mode", "0", "How Gun Toss will be played: 0 - no double-dropping checking, deagle gets 7 ammo at start, 1 - double dropping check, deagle gets 7 ammo on drop, colouring of deagles, deagle markers", 0);
	gShadow_LR_GunToss_StartMode = 1;
	gH_Cvar_LR_GunToss_ShowMeter = CreateConVar("sm_hosties_lr_gt_meter", "0", "Displays a distance meter: 0 - do not display, 1 - display", 0, true, 0.0, true, 1.0);
	...
	gH_Cvar_LR_ChickenFight_Rebel = CreateConVar("sm_hosties_lr_cf_cheat_action", "0", "What to do with a chicken fighter who attacks the other player with another weapon than knife: 0 - abort LR, 1 - slay player", 0, true, 0.0, true, 1.0);
	...
	gH_Cvar_LR_M4M_MagCapacity = CreateConVar("sm_hosties_lr_m4m_capacity", "2", "The number of bullets in each magazine given to Mag4Mag LR contestants", 0, true, 2.0);
	gShadow_LR_M4M_MagCapacity = 7;
	gH_Cvar_LR_KnifeFight_LowGrav = CreateConVar("sm_hosties_lr_kf_gravity", "0.4", "The multiplier used for the low-gravity knife fight.", 0, true, 0.0, true, 1.0);
	gShadow_LR_KnifeFight_LowGrav = 0.6;
	gH_Cvar_LR_KnifeFight_HiSpeed = CreateConVar("sm_hosties_lr_kf_speed", "1.5", "The multiplier used for the high-speed knife fight.", 0, true, 1.1);
	gShadow_LR_KnifeFight_HiSpeed = 2.2;
	gH_Cvar_LR_KnifeFight_Drunk = CreateConVar("sm_hosties_lr_kf_drunk", "10", "The multiplier used for how drunk the player will be during the drunken boxing knife fight.", 0, true, 0.0);
	...
	gH_Cvar_LR_BlockSuicide = CreateConVar("sm_hosties_lr_blocksuicide", "1", "Blocks LR participants from commiting suicide to avoid deaths: 0 - disable, 1 - enable", 0, true, 0.0, true, 1.0);
...
public Action:Timer_Countdown(Handle:timer)
{
	...
	for (new idx = 0; idx < GetArraySize(gH_DArray_LR_Partners); idx++)
	{
		...
		if (type == LR_NoScope && countdown == 3)
		{
			new NoScopeWeapon:NS_Selection;
			NS_Selection = GetArrayCell(gH_DArray_LR_Partners, idx, _:Block_Global2);					
			new NSW_Prisoner, NSW_Guard;
		...
		{
			bCountdownUsed = true;
			PrintCenterText(LR_Player_Prisoner, "A játék elkezdődik %i másodperc múlva", countdown);
			PrintCenterText(LR_Player_Guard, "A játék elkezdődik %i másodperc múlva", countdown);
