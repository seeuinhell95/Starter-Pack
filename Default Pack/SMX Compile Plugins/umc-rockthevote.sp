...
{
    name        = "[CSGO] UMC - Rock The Vote",
    author      = "Steell, PowerLord & Mr. Silence | Edited: somebody.",
    description = "UMC - Rock The Vote",
    version     = "1.0",
...
new bool:rtv_completed;
new bool:rtv_enabled;
new bool:vote_completed;
new bool:rtv_inprogress;
...
public OnPluginStart()
{
    ...
    AutoExecConfig(true, "umc/umc-rockthevote");
...
AttemptRTV(client)
{
    ...
    {
        PrintToChat(client, "[\x04UMC\x01] %t","No RTV Admin");
        return;
    }
    ...
    {
        PrintToChat(client, "[\x04UMC\x01] %t", "No RTV Nextmap");
        return;
    }
    ...
    {
        PrintToChat(client, "[\x04UMC\x01] %t", "No RTV Player Count", minPlayers - clients);
        return;
    }
    //Otherwise, print a message if it is too early to RTV.
    else if (rtv_delaystart > 0)
    {
        PrintToChat(client, "[\x04UMC\x01] %t", "No RTV Time", rtv_delaystart);
        return;
    }
    ...
    else if (FindValueInArray(rtv_clients, client) == -1)
    {
        ...
        if (size >= rtv_threshold)
        {
            ...
            {
                PrintToChatAll("[\x04UMC\x01] %t", "RTV Start");
                StartRTV();
            }
            else //Otherwise, display a message.
            {
                PrintToChat(client, "[\x04UMC\x01] %t", "Vote In Progress");
