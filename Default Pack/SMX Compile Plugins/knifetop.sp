...
public void OnPluginStart()
{
	...
	gCV_Enabled = CreateConVar("knifetop_enable", "1", "Is the plugin enabled?", _, true, 0.0, true, 1.0);
	gCV_Team = CreateConVar("knifetop_team", "0", "Which teams should have knife kills counted?\n0 - Everyone\n2 - Terroists\n3 - Counter-Terrorists", _, true, 0.0, true, 3.0);
	gCV_HostiesLR = CreateConVar("knifetop_hosties_lr", "0", "In effect only if SM_Hosties 2 is installed on the server!\nIgnore knifetop_team if a knife kill was done between 2 players in a last request?", _, true, 0.0, true, 1.0);
	gCV_TopLimit = CreateConVar("knifetop_top_limit", "100", "Amount of people to display when using sm_knifetop.", _, true, 1.0);

	AutoExecConfig(true, "knifetop");
