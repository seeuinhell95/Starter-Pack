...
{
    name        = "[CSGO] UMC - Nominations",
    author      = "Steell, PowerLord & Mr. Silence | Edited: somebody.",
    description = "UMC - Nominations",
    version     = "1.0",
...
public OnPluginStart()
{
    ...
    AutoExecConfig(true, "umc/umc-nominate");
...
public Action:OnPlayerChat(client, const String:command[], argc)
{
    ...
    if (StrEqual(arg, "nominate", false))
    {
        ...
        {
            PrintToChat(client, "[\x04UMC\x01] %t", "No Nominate Nextmap");
        }
            ...
            if (next != -1)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        PrintToChatAll("[\x04UMC\x01] %t", "Player Nomination", clientName, arg);
...
public Action:Command_Nominate(client, args)
{
    ...
    if (vote_completed || !can_nominate)
    {
        ReplyToCommand(client, "[\x04UMC\x01] %t", "No Nominate Nextmap");
    }
    else //Otherwise, let them nominate.
    {
        if (args > 0)
...
public Handle_NominationMenu(Handle:menu, MenuAction:action, client, param2)
{
    switch (action)
    {
        case MenuAction_Select: //The client has picked something.
        {
            ...
            decl String:clientName[MAX_NAME_LENGTH];
            GetClientName(client, clientName, sizeof(clientName));
            PrintToChatAll("[\x04UMC\x01] %t", "Player Nomination", clientName, map);
