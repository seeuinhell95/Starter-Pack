...
{
	name		= "[CSGO] RainBow Chat",
	author		= "Kyle | Edited: somebody.",
...
{
	g_cvarName = CreateConVar("src_name_flag", "abz", "Set flag for vip must have to get access to rainbow chat name feature", FCVAR_NOTIFY);
	g_cvarMesasge = CreateConVar("src_message_flag", "abz", "Set flag for vip must have to get access to rainbow chat messge feature", FCVAR_NOTIFY);

	HookConVarChange(g_cvarName, HookConVar);
	HookConVarChange(g_cvarMesasge, HookConVar);

	AutoExecConfig(true, "rainbowchat");

	RegConsoleCmd("sm_rainbowname", Command_Name);
	RegConsoleCmd("sm_rbn", Command_Name);

	RegConsoleCmd("sm_rainbowchat", Command_Message);
