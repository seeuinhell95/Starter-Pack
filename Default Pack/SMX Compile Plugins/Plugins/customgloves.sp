...
{
	name = "[CSGO] Custom Gloves & Arms",
	author = "Franc1sco | Edited: somebody.",
	description = "Custom Gloves & Arms",
	version = "1.0",
...
public OnPluginStart()
{
	...
	RegConsoleCmd("sm_glove", Command_);
	RegConsoleCmd("sm_gl", Command_);
	RegConsoleCmd("sm_customgloves", Command_);
	RegConsoleCmd("sm_customglove", Command_);
	RegConsoleCmd("sm_customgl", Command_);
	RegConsoleCmd("sm_cgloves", Command_);
	RegConsoleCmd("sm_cglove", Command_);
	RegConsoleCmd("sm_cgl", Command_);
	RegConsoleCmd("sm_arms", Command_);
	RegConsoleCmd("sm_arm", Command_);

	RegAdminCmd("sm_reloadarms", ReloadSkins, ADMFLAG_ROOT);
	RegAdminCmd("sm_reloadgloves", ReloadSkins, ADMFLAG_ROOT);
...
{	
	RefreshKV();
	ReplyToCommand(client, "[SM] Custom Gloves Menu Configuration Reloaded!");
...
{
	BuildPath(Path_SM, sConfig, PLATFORM_MAX_PATH, "configs/custom_gloves/gloves.txt");
...
Downloads()
{
	...
	BuildPath(Path_SM, imFile, sizeof(imFile), "configs/custom_gloves/downloads.txt");
	...
	{
		LogError("[SM] no file found for downloads (configs/custom_gloves/downloads.txt)");
	}
}
...
{	
	...
	SetMenuTitle(menu, "Válassz csapatkesztyűket");
	if(GetFeatureStatus(FeatureType_Native, "ZR_IsClientZombie") == FeatureStatus_Available)
	{
		AddMenuItem(menu, "option3", "Válassz Zombi kezet");
		AddMenuItem(menu, "option4", "Válassz Ember kezet");
	}
	else
	{
		AddMenuItem(menu, "option1", "Válassz CT kesztyűt");
		AddMenuItem(menu, "option2", "Válassz T kesztyűt");
	}
...
ShowMenuCTs(client, item)
{
	...
	SetMenuTitle(clientmenu[client][3], "Egyedi Csapatkesztyű Menü\nVálassz CT csapatkesztyűt");
...
ShowMenuTTs(client, item)
{
	...
	SetMenuTitle(clientmenu[client][2], "Csapatkesztyű Menü\nVálassz T csapatkesztyűt");
...
ShowMenuZombies(client, item)
{
	...
	SetMenuTitle(clientmenu[client][0], "Egyedi Csapatkesztyű Menü\nVálassz Zombi kezet");
...
ShowMenuHumans(client, item)
{
	...
	SetMenuTitle(clientmenu[client][1], "Egyedi Csapatkesztyű Menü\nVálassz Ember kezet");
