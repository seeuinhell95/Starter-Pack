...
{
	name = "[CSGO] Weapons & Knives",
	author = "KGNS | Edited: somebody.",
	description = "Weapons & Knives",
	version = "1.0",
...
public void OnPluginStart()
{
	...
	g_Cvar_EnableWeaponOverwrite 	= CreateConVar("sm_weapons_enable_overwrite", 		"0", 				"Enable/Disable players overwriting other players' weapons (picked up from the ground) by using !ws command");
...
public Action CommandWeaponSkins(int client, int args)
{
	if (IsValidClient(client))
	{
		...
		{
			PrintToChat(client, " \x02%s \x06%t", g_ChatPrefix, "GracePeriod", g_iGracePeriod);
		}
	}
...
public Action CommandSeedMenu(int client, int args)
{
	if(!g_bEnableSeed)
	{
		ReplyToCommand(client, " \x02%s \x06%T", g_ChatPrefix, "SeedDisabled", client);
		return Plugin_Handled;
	}
	ReplyToCommand(client, " \x02%s \x06%T", g_ChatPrefix, "SeedExplanation", client);
	return Plugin_Handled;
}
...
public Action CommandNameTag(int client, int args)
{
	if(!g_bEnableNameTag)
	{
		ReplyToCommand(client, " \x02%s \x06%T", g_ChatPrefix, "NameTagDisabled", client);
		return Plugin_Handled;
	}
	ReplyToCommand(client, " \x02%s \x06%T", g_ChatPrefix, "NameTagNew", client);
	return Plugin_Handled;
}
