...
public Plugin myinfo =
{
	...
	author = "PowerLord, TotenFluch, AlliedModders LLC & Wesker | Edited: somebody.",
...
AttemptRTV(client)
{
	...
	{
		C_ReplyToCommand(client, "[SM] %t", "RTV Not Allowed");
		return;
	}

	if (!CanMapChooserStartVote())
	{
		C_ReplyToCommand(client, "[SM] %t", "RTV Started");
		return;
	}

	if (GetClientCount(true) < GetConVarInt(g_Cvar_MinPlayers))
	{
		C_ReplyToCommand(client, "[SM] %t", "Minimal Players Not Met");
		return;			
	}

	if (g_Voted[client])
	{
		C_ReplyToCommand(client, "[SM] %t", "Already Voted", g_Votes, g_VotesNeeded);
		return;
	}	
