...
{
	name = "[CSGO] MapChooser Extended - Core",
	author = "PowerLord, Zuko, TotenFluch, SM9CC, AlliedModders LLC & Wesker | Edited: somebody.",
	description = "MapChooser Extended - Core",
	version = "1.0",
...
char g_NextMapString[PLATFORM_MAX_PATH];
...
public OnPluginStart()
{
	...
	g_Cvar_ExtendRoundStep = CreateConVar("mce_extend_roundstep", "9999", "Specifies how many more rounds each extension makes", _, true, 1.0);
	g_Cvar_ExtendFragStep = CreateConVar("mce_extend_fragstep", "9999", "Specifies how many more frags are allowed when map is extended.", _, true, 5.0);	
	g_Cvar_ExcludeMaps = CreateConVar("mce_exclude", "2", "Specifies how many past maps to exclude from the vote.", _, true, 0.0);
	...
	g_Cvar_Extend = CreateConVar("mce_extend", "3", "Number of extensions allowed each map.", _, true, 0.0);
	g_Cvar_DontChange = CreateConVar("mce_dontchange", "0", "Specifies if a 'Don't Change' option should be added to early votes", _, true, 0.0);
	...
	g_Cvar_NoVoteOption = CreateConVar("mce_addnovote", "0", "Add \"No Vote\" to vote menu?", _, true, 0.0, true, 1.0);
	...
	BuildPath(Path_SM, mapListPath, PLATFORM_MAX_PATH, "configs/mapchooser/%s.txt", g_GameModName);
...
public APLRes:AskPluginLoad2(Handle:myself, bool:late, String:error[], err_max)
{
	...
	CreateNative("SetEndOfMapVoteFinished", Native_SetVoteDone);
...
public Action:Command_SetNextmap(client, args)
{
	if (args < 1)
	{
		CPrintToChat(client, "[MCE] Usage: sm_setnextmap <map>");
	...
	if (!IsMapValid(map))
	{
		CPrintToChat(client, "[MCE] %t", "Map was not found", map);
		return Plugin_Handled;
	}
...
public Action Timer_ChangeMap(Handle hTimer, Handle dp)
{
	...
	{
		ResetPack(dp);
		ReadPackString(dp, g_NextMapString, sizeof(g_NextMapString));
	}
	
	LogAction(-1, -1, "MCE change map");
	CreateTimer(0.5, Timer_Final);
}

public Action Timer_Final(Handle hTimer, any data)
{
	ForceChangeLevel(g_NextMapString, "Map Vote");
}
...
public Native_SetVoteDone(Handle:plugin, numParams)
{
	g_MapVoteCompleted = true;
}
