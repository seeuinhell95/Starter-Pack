...
public Action ChatListener(int client, const char[] command, int args)
{
	...
	else if (g_bWaitingForNametag[client] && IsValidClient(client) && g_iIndex[client] > -1 && !IsChatTrigger())
	{
		...
		{
			PrintToChat(client, " \x02%s \x06%t", g_ChatPrefix, "NameTagCancelled");
			return Plugin_Handled;
		}
		...
		PrintToChat(client, " \x02%s \x06%t: \x0E\"%s\"", g_ChatPrefix, "NameTagSuccess", msg);
	...
	else if (g_bWaitingForSeed[client] && IsValidClient(client) && g_iIndex[client] > -1 && !IsChatTrigger())
	{
		...
		{
			PrintToChat(client, " \x02%s \x06%t", g_ChatPrefix, "SeedCancelled");
			return Plugin_Handled;
		}
		else if ((seedInt = StringToInt(msg)) < 0 || seedInt > 8192)
		{
			PrintToChat(client, " \x02%s \x06%t", g_ChatPrefix, "SeedFailed");
			return Plugin_Handled;
		}
		...
		PrintToChat(client, " \x02%s \x06%t: \x01%i", g_ChatPrefix, "SeedSuccess", seedInt);
