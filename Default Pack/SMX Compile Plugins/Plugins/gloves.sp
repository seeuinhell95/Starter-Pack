...
{
	name = "[CSGO] Valve Gloves",
	author = "KGNS | Edited: somebody.",
	description = "Valve Gloves",
	version = "1.0",
...
public void OnPluginStart()
{
	...
	g_Cvar_ChatPrefix = CreateConVar("sm_gloves_chat_prefix", "[GL]", "Prefix for chat messages");
	g_Cvar_EnableFloat = CreateConVar("sm_gloves_enable_float", "1", "Enable/Disable gloves float options");
	g_Cvar_FloatIncrementSize = CreateConVar("sm_gloves_float_increment_size", "0.05", "Increase/Decrease by value for gloves float");
	g_Cvar_EnableWorldModel = CreateConVar("sm_gloves_enable_world_model", "0", "Enable/Disable gloves to be seen by other living players");
	...
	RegConsoleCmd("sm_vgloves", CommandGlove);
	RegConsoleCmd("sm_vglove", CommandGlove);
	RegConsoleCmd("sm_vgl", CommandGlove);
	RegConsoleCmd("sm_valvegloves", CommandGlove);
	RegConsoleCmd("sm_valveglove", CommandGlove);
	RegConsoleCmd("sm_valvegl", CommandGlove);

	RegConsoleCmd("sm_gloveslang", CommandGloveLang);
	RegConsoleCmd("sm_glovelang", CommandGloveLang);
