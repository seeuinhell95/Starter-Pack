...
public void OnPluginStart()
{
	...
	RegAdminCmd("sm_wasted", Command_Wasted, ADMFLAG_RESERVATION, "Wasted command");
...
public Action DOMenu(int client, int args)
{
	...
	else
	{
		...
		menu.SetTitle("Szerver Játékidő - Összes");
...
public int SQLShowPlayTime(Handle owner, Handle hndl, char [] error, any data)
{
	...
	menu.SetTitle("Szerver játékidő: %s\n ", name);
	...
	Format(buffer, 124, "Összes játékidő: %s", buffer);
	menu.AddItem("", buffer, ITEMDRAW_DISABLED);
...
public int DIDMenuHandler(Menu menu, MenuAction action, int client, int itemNum) 
{
	if( action == MenuAction_Select ) 
	{
		...
		if( strcmp(info,"option1") == 0 )
		{
			...
			menu2.SetTitle("Szerver játékidő: %N\n ", client);
...
public int SQLShowWasteTime(Handle owner, Handle hndl, char [] error, any client)
{
	...
	while (SQL_FetchRow(hndl))
	{
		char buffer[124];
		ShowTimer(SQL_FetchInt(hndl, 0), buffer, sizeof(buffer));
		PrintToChatAll(" \x02[\x0BActive\x02] \x06Összesen \x02%s \x06időt töltöttek eddig a játékosok a szerveren.", buffer);
	}
