...
{
	name = "[CSGO] Skins Chooser",
	author = "Andi67 | Edited: somebody.",
	description = "Skins Chooser",
	version = "1.0",
...
public void OnPluginStart()
{
	...
	g_steamid  = CreateConVar("sm_skinchooser_steamid_format", "0", "0 = SteamId 2 , 1 = SteamId 3", _, true, 0.0, true, 1.0);
	g_mapbased = CreateConVar("sm_skinchooser_mapbased", "0", "0 = Disabled , 1 = Enables usage of mapbased inis.", _, true, 0.0, true, 1.0);	
	g_autodisplay = CreateConVar("sm_skinchooser_autodisplay", "0", "0 = Disabled , 1 = Enables Menu Auto popup.", _, true, 0.0, true, 1.0);
	...
	g_ForcePlayerSkin = CreateConVar("sm_skinchooser_forceplayerskin", "1", "0 = Disabled , 1 = Enabled , should Players get automaticly a Model?", _, true, 0.0, true, 1.0);
	...
	g_SkinAdmin = CreateConVar("sm_skinchooser_skinadmin", "1", "0 = Disabled , 1 = Enabled , should Admins get automaticly a Model?", _, true, 0.0, true, 1.0);	
	...
	g_blockAgentModels = CreateConVar("sm_skinchooser_blockagentmodels", "0", "0 = Disabled , 1 = Enabled , blocks the new AgentModels.", _, true, 0.0, true, 1.0);	
	...
	RegConsoleCmd("sm_agent", Command_Model);
	RegConsoleCmd("sm_agents", Command_Model);
	...
	AutoExecConfig(true, "skinchooser");	
}
...
public void OnMapStart()
{
	if(GetConVarInt(g_enabled) == 1)
	{
		...
		if(GetConVarInt(g_mapbased) == 1)	
		{
			...
			{
				BuildPath(Path_SM, file, sizeof(file), "configs/skinchooser/%s_skins.ini", curmap[19]);			
				BuildPath(Path_SM, files, sizeof(files), "configs/skinchooser/%s_skins_downloads.ini", curmap[19]);
			}
			else
			{
				BuildPath(Path_SM, file, sizeof(file), "configs/skinchooser/%s_skins.ini", curmap);
				BuildPath(Path_SM, files, sizeof(files), "configs/skinchooser/%s_skins_downloads.ini", curmap);	
			}

			if (!FileExists(file))
			{
				BuildPath(Path_SM, file, sizeof(file), "configs/skinchooser/skins.ini");
			}

			if (!FileExists(files))
			{			
				BuildPath(Path_SM, files, sizeof(files), "configs/skinchooser/skins_downloads.ini");
			}
			...
			{			
				if (strncmp(curmap, "workshop", 8) == 0)
				{			
					BuildPath(Path_SM, filea, sizeof(filea), "configs/skinchooser/%s_arms.ini", curmap[19]);
					BuildPath(Path_SM, fileb, sizeof(fileb), "configs/skinchooser/%s_arms_downloads.ini", curmap[19]);			
				}	
				else
				{		
					BuildPath(Path_SM, filea, sizeof(filea), "configs/skinchooser/%s_arms.ini", curmap);
					BuildPath(Path_SM, fileb, sizeof(fileb), "configs/skinchooser/%s_arms_downloads.ini", curmap);		
				}

				if (!FileExists(filea))
				{
					BuildPath(Path_SM, filea, sizeof(filea), "configs/skinchooser/arms.ini");
				}			
				if (!FileExists(fileb))
				{						
					BuildPath(Path_SM, fileb, sizeof(fileb), "configs/skinchooser/arms_downloads.ini");
				}
			}
		}
		...
		{	
			...
			g_ModelsPlayer_Count_Team2 = LoadModels(g_ModelsPlayerTeam2, "configs/skinchooser/forceplayer_terrorist.ini");
			g_ModelsPlayer_Count_Team3  = LoadModels(g_ModelsPlayerTeam3,  "configs/skinchooser/forceplayer_counterterrorist.ini");
		}
		if(GetConVarInt(g_SkinBots) == 1)
		{
			g_ModelsBots_Count_Team2 = 0;
			g_ModelsBots_Count_Team3 = 0;
			g_ModelsBots_Count_Team2 = LoadModels(g_ModelsBotsTeam2, "configs/skinchooser/forcebot_terrorist.ini");
			g_ModelsBots_Count_Team3  = LoadModels(g_ModelsBotsTeam3,  "configs/skinchooser/forcebot_counterterrorist.ini");
		}
		if(GetConVarInt(g_SkinAdmin) == 1)	
		{		
			g_ModelsAdmin_Count_Team2 = 0;
			g_ModelsAdmin_Count_Team3 = 0;
			g_ModelsAdmin_Count_Team2 = LoadModels(g_ModelsAdminTeam2, "configs/skinchooser/forceadmin_terrorist.ini");
			g_ModelsAdmin_Count_Team3  = LoadModels(g_ModelsAdminTeam3,  "configs/skinchooser/forceadmin_counterterrorist.ini");
		}
	}
...
Handle BuildMainMenu(int client)
{
	...
	AddMenuItem(menu,"none","Pálya Kinézet");
	SetMenuTitle(menu, "Kinézetek");
...
stock void CommandSecMenu(int param1)
{
...
Handle BuildArmsMainMenu(int param1)
{
			...
			AddMenuItem(secmenu,"none","Pálya Kéz");
			SetMenuTitle(secmenu, "Kezek");
	...
	if (action == MenuAction_Select)
	{
		...
		if (!StrEqual(info,"") && IsModelPrecached(info) && IsClientConnected(param1))
		{
			if(IsPlayerAlive(param1)) 	
			{
				...
				RequestFrame(RemoveItem, EntIndexToEntRef(param1));
			}
		}
...
public Action Timer_Menu(Handle timer, any client)
{
	...
	PrintToChat(client, "[SM] A kinézet menü megnyitva, válassz egy kinézetet.");
	return Plugin_Handled;
}
...
public Action Event_PlayerSpawn(Handle event,  const char[] name, bool dontBroadcast)
{
	...
	{	
		...
		if (StrEqual(Game, "csgo") && GetConVarInt(g_arms_enabled) == 0)
		{							
		...
		if (!IsFakeClient(client) && IsValidClient(client) && GetConVarInt(g_SkinAdmin) == 1)	
		{
			...
			{	
				if (GetConVarInt(g_SkinAdminTimerEnabled) == 1)	
				{				
					CreateTimer(GetConVarFloat(g_SkinAdminTimer), skin_admin, client);
				}
				else
				{
					if (team==2)
		...
		{		
			...
			if(admin == INVALID_ADMIN_ID)	
			{
				if (GetConVarInt(g_ForcePlayerSkinTimerEnabled) == 1)		
				{				
					CreateTimer(GetConVarFloat(g_ForcePlayerSkinTimer), skin_players, client);
				}
				else
				{
					if (team==2)
		...
		if (StrEqual(Game, "csgo") && GetConVarInt(g_arms_enabled) == 1)
		{
			...
			{			
  				RequestFrame(PlayerModel,client);  				
  			}
  		}
  		else
  		{
			if (!IsFakeClient(client) && IsValidClient(client))
			{  			
  				RequestFrame(Timer_Spawn, client);
  			}
  		}
  	}
}

public void PlayerModel(int client)
{
	// Get the user's authid			
...
public void Timer_Spawn(int client)
{
	// Get the user's authid	
	KvJumpToKey(playermodelskv,authid[client],true);
...
public void skin_bots(int client)
{
	int team = GetClientTeam(client);
	if (team==2)
	{
...
stock void SetDefaultModels(int client) 
{
	...
	{			
		...
		{			
			...
			if(GetConVarInt(g_blockAgentModels) == 1)	
			{			
				for (int h = 0; h < sizeof(AgentModelsT); h++) 
				{
    				if (StrEqual(AgentModelsT[h], sModelT))
    				{
   				 		SetEntityModel(client,phoenixModelsT[GetRandomInt(0 , -1)]);
						PrintToChat(client, "[SM] A szerveren az egyéni Valve (Agents) kinézetek le vannak tiltva!");   				 		
  					}
  				}  					
			}	
		...
		{			
			...
			if(GetConVarInt(g_blockAgentModels) == 1)	
			{			
				for (int o = 0; o < sizeof(AgentModelsCT); o++) 
				{
    				if (StrEqual(AgentModelsCT[o], sModelCT))
    				{
   				 		SetEntityModel(client,sasModelsCT[GetRandomInt(0 , -1)]);
						PrintToChat(client, "[SM] A szerveren az egyéni Valve (Agents) kinézetek le vannak tiltva!");
   				 	}   				 		
				}
			}
