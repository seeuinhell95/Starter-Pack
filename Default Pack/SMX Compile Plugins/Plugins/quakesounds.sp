...
public Plugin myinfo =
{
	...
	author = "Spartan_C001 & JoinedSenses | Edited: somebody.",
	description = "Quake Sounds",
	version = "1.0",
...
{
	g_cvarAnnounce = CreateConVar("sm_quakesoundsv3_announce", "0", "Sets whether to announcement to clients as they join, 0=Disabled, 1=Enabled.", FCVAR_NONE, true, 0.0, true, 1.0);
	g_cvarText = CreateConVar("sm_quakesoundsv3_text", "0", "Default text display setting for new users, 0=Disabled, 1=Enabled.", FCVAR_NONE, true, 0.0, true, 1.0);
	g_cvarSound = CreateConVar("sm_quakesoundsv3_sound", "0", "Default sound set for new users, 0=Disabled, 1=Standard, 2=Female.", FCVAR_NONE, true, 0.0);
	...
	RegConsoleCmd("sm_quakesound", CMD_ShowQuakePrefsMenu);
	RegConsoleCmd("sm_quakesounds", CMD_ShowQuakePrefsMenu);

	AutoExecConfig(true, "quakesounds");
	LoadTranslations("quakesounds.phrases");
	...
	SetCookieMenuItem(QuakePrefSelected, 0, "Quake hangok");
...
public void LoadSoundSets() {
	...
	BuildPath(Path_SM, bufferString, PLATFORM_MAX_PATH, "configs/quake/quakesetslist.cfg");

	if (!SoundSetsKV.ImportFromFile(bufferString)) {
		delete SoundSetsKV;
		SetFailState("configs/quake/quakesetslist.cfg not found");
	}

	if (!SoundSetsKV.JumpToKey("sound sets")) {
		delete SoundSetsKV;
		SetFailState("configs/quake/quakesetslist.cfg not correctly structured");
	}
...
public void ShowQuakeMenu(int client) {
	...
	Format(buffer, 100, (g_iSoundPreference[client] == -1) ? "%T (X)" : "%T", "no quake sounds", client);
	menu.AddItem("no sounds", buffer);

	for (int set = 0; set < g_iNumSets; set++) {
		Format(buffer, 50, (g_iSoundPreference[client] == set) ? "%T (X)" : "%T", g_sSetsName[set], client);
