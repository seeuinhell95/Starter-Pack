...
{
	name = "[CSGO] Stop Map Music Volume",
	author = "Mitch & Agent Wesker | Edited: somebody.",
	description = "Stop Map Music Volume",
	version = "1.0",
...
{
	RegConsoleCmd("sm_stopmapmusic", Command_StopMusic, "Toggles map music");
	RegConsoleCmd("sm_stopmusic", Command_StopMusic, "Toggles map music");
	RegConsoleCmd("sm_stop", Command_StopMusic, "Toggles map music");
	RegConsoleCmd("sm_music", Command_Music, "Toggles map music menu");
...
public Action ClientMusicNotice(Handle timer, int client)
{
	...
	{
		PrintToChat(client, " \x04[STOP] \x02Pályazenék jelenleg kikapcsolva, írd be \x0E!music \x02a beállításokhoz.");
	}
}
...
public Action Command_StopMusic(int client, any args)
{
	...
	PrintToChat(client, " \x04[STOP] \x02Pályazene leállítva ~ Írd be \x0E!music \x02vagy \x0E!volume \x02a beállításokért.");
...
public int Volume_Menu(Menu menu, MenuAction action, int client, int param)
{
	if (action == MenuAction_Select && IsValidClient(client))
	{
		...
		PrintToChat(client, " \x04[STOP] \x02A hangerő a következő zenénél megváltozik.");
...
static void makeMusicMenu(int client)
{
	...
	if (g_bDisabled[client]) {
		toggleSelection = "Kikapcsolva";
	} else {
		toggleSelection = "Bekapcsolva";
	}
	
	musicMenu.SetTitle("Pályazene: %s | Hangerő: %i%", toggleSelection, RoundFloat(g_fClientVol[client]*100));
	musicMenu.AddItem("off", "Kikapcsolás");
	musicMenu.AddItem("on", "Bekapcsolás");
	musicMenu.AddItem("spacer1", "spacer1", ITEMDRAW_SPACER);
	musicMenu.AddItem("vol", "Hangerő");
...
static void makeVolumeMenu(int client)
{
	...
	volumeMenu.SetTitle("Hangerő: %i%", RoundFloat(g_fClientVol[client]*100));
