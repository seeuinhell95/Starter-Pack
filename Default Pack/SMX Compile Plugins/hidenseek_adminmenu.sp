...
#define AUTHOR							"ceLoFaN | Edited: somebody."
...
#define FLASHBANG_CHANCE				"0.75"
#define MOLOTOV_CHANCE					"0.40"
#define SMOKE_CHANCE					"0.0"
#define DECOY_CHANCE					"0.60"
#define HE_CHANCE						"0.50"
...
{
    name = "[CSGO] HideNSeek + Admin Menu (ChaseMod)",
    author = AUTHOR,
    description = "HideNSeek + Admin Menu (ChaseMod)",
...
public void OnPluginStart()
{
    ...
    AutoExecConfig(true, "hidenseek_adminmenu");
...
public Action Command_ToggleKnife(int iClient, int args)
{
    if(iClient > 0 && iClient <= MaxClients && IsClientInGame(iClient)) {
        ...
        PrintToChat(iClient, " \x04[\x02HNS\x04] \x06%t", g_baToggleKnife[iClient] ? "Toggle Knife On" : "Toggle Knife Off");
        ...
        if(IsClientInGame(iTarget)) {
            ...
            if(iClientTeam != iTargetTeam && iTargetTeam != CS_TEAM_SPECTATOR) {
                ...
                if(GetVectorDistance(faTargetCoord, faClientCoord) <= 500) {
                    PrintToChat(iClient, " \x04[\x02HNS\x04] \x06%t", "Respawn Aborted Enemy Near");
                    return false;
                }
            }
        }
...
public Action Command_Respawn(int iClient, int args)
{
    if(iClient > 0 && iClient <= MaxClients && IsClientInGame(iClient)) {
        if(!g_bRespawnMode)
            PrintToChat(iClient, " \x04[\x02HNS\x04] \x06%t", "Respawn Aborted Off");
        else if(IsPlayerRespawning(iClient))
            PrintToChat(iClient, " \x04[\x02HNS\x04] \x06%t", "Respawn Aborted Alive");
        else if(!(GetEntityFlags(iClient) & FL_ONGROUND))
            PrintToChat(iClient, " \x04[\x02HNS\x04] \x06%t", "Respawn Aborted In Flight");
        else {
...
void MakeClientInvisible(int iClient, float fDuration)
{
    ...
    PrintToChat(iClient, " \x04[\x02HNS\x04] \x06%t", "Invisible On", fDuration);
...
public Action MakeClientVisible(Handle hTimer, any iClient)
{
    ...
    if(IsPlayerAlive(iClient))
        PrintToChat(iClient, " \x04[\x02HNS\x04] \x06%t", "Invisible Off");
}
...
void BreakInvisibility(int iClient, int iReason)
{
    if(g_haInvisible[iClient] != null) {
        ...
        if(iReason == REASON_ENEMY_TOO_CLOSE)
            PrintToChat(iClient, " \x04[\x02HNS\x04] \x06%t", "Invisibility Broken Enemy Near");
        else if(iReason == REASON_LADDER)
            PrintToChat(iClient, " \x04[\x02HNS\x04] \x06%t", "Invisibility Broken Climb");
        else if(iReason == REASON_GRENADE)
            PrintToChat(iClient, " \x04[\x02HNS\x04] \x06%t", "Invisibility Broken Threw Grenade");
    }
}
...
public Action OnPlayerSpawnDelay(Handle hTimer, any iId)
{
    ...
    if(IsClientInGame(iClient) && IsPlayerAlive(iClient)) {
        ...
        if(iTeam == CS_TEAM_T) {
            ...
            if(IsValidEntity(iWeapon)) {
                SetEntPropFloat(iWeapon, Prop_Send, "m_flNextPrimaryAttack", GetGameTime() + 9001.0);
...
public Action HandlePlayerSpectateRequest(int iClient) {
    ...
    if(g_bRespawnMode) {
        if(iTeam == CS_TEAM_T)
            if(PlayerCanDisappear(iClient)) {
                PrintToChat(iClient, " \x04[\x02HNS\x04] \x06%t", "Spectate Pending", 5.0, "s");
...
public Action Command_JoinTeam(int iClient, const char[] sCommand, int iArgCount)
{
    ...
    if(iTeam == CS_TEAM_T || iTeam == CS_TEAM_CT) {
        if(iChosenTeam == JOINTEAM_T || iChosenTeam == JOINTEAM_CT || iChosenTeam == JOINTEAM_RND) {
            if(IsPlayerAlive(iClient)) {
                PrintToChat(iClient, " \x04[\x02HNS\x04] \x06%t", "Team Change Deny Alive");
                return Plugin_Stop;
            }
                ...
                PrintToChat(iClient, " \x04[\x02HNS\x04] \x06%t", "Team Change Deny Balanced");
                return Plugin_Stop;
            }
        }
        ...
        if(iDelta > iLimitTeams || -iDelta > iLimitTeams) {
            ...
            PrintToChat(iClient, " \x04[\x02HNS\x04] \x06%t", (g_iaInitialTeamTrack[iClient] == CS_TEAM_T) ? "Assigned To Team T" : "Assigned To Team CT");
        ...
        if(iVictimTeam == CS_TEAM_T) {
            ...
            if(iAttacker > 0 && iAttacker <= MaxClients) {
                ...
                if(iAttackerTeam == CS_TEAM_CT) {
                    SetEntProp(iAttacker, Prop_Send, "m_iAccount", 0);
                    ...
                    if(!g_bRespawnMode)
                        PrintToChat(iAttacker, " \x04[\x02HNS\x04] \x06%t",
                        ...
                        PrintToChat(iVictim, " \x04[\x02HNS\x04] \x06%t", "Killing Notify Victim", sNickname);
                    }
                }
            }
            else {
                ...
                if(g_iSuicidePointsPenalty) {
                    ...
                    PrintToChat(iVictim, " \x04[\x02HNS\x04] \x06%t", "Died By Falling", g_iSuicidePointsPenalty);
                }
                if(g_bRespawnMode) {
                    if(!g_baDiedBecauseRespawning[iVictim]) {
                        ...
                        if(iVictimTeamBalanceStatus & (TEAM_NEEDS_NERF | TEAM_CAN_TAKE_NERF)) {
                            ...
                            PrintToChat(iVictim, " \x04[\x02HNS\x04] \x06%t", "Assigned To Team CT");
                        }
                    }
                    else {
                        g_baAvailableToSwap[iVictim] = false;
                ...
                if(g_bRespawnMode) {
                    if(!g_baDiedBecauseRespawning[iVictim]) {
                        ...
                        if(iVictimTeamBalanceStatus & TEAM_NEEDS_NERF) {
                            ...
                            PrintToChat(iVictim, " \x04[\x02HNS\x04] \x06%t", "Assigned To Team T");
                        }
                        else {
                            g_baAvailableToSwap[iVictim] = false;
                        }
            ...
            if(!IsPlayerAlive(iTarget)) {
                ...
                if((iClientTeam == CS_TEAM_CT && iTargetTeam == CS_TEAM_T) || (iClientTeam == CS_TEAM_T && iTargetTeam == CS_TEAM_CT))
                    if(g_baAvailableToSwap[iTarget]) {
                        ...
                        PrintToChat(iClient, " \x04[\x02HNS\x04] \x06%t", "Swapped", sNickname);

                        GetClientName(iClient, sNickname, sizeof(sNickname));
                        PrintToChat(iTarget, " \x04[\x02HNS\x04] \x06%t", "Swapped", sNickname);
                    }
            }
    }
}
...
public void OnRoundEnd(Event hEvent, const char[] name, bool dontBroadcast)
{
    ...
    if(iWinningTeam == CS_TEAM_T) {
        ...
        if(!g_iMaximumWinStreak || g_iTWinsInARow < g_iMaximumWinStreak)
            PrintToChatAll(" \x04[\x02HNS\x04] \x06%t", "T Win");
        else {
            SwapTeams();
            g_iTWinsInARow = 0;
            ...
            if(g_iMaximumWinStreak)
                PrintToChatAll(" \x04[\x02HNS\x04] \x06%t", "T Win Team Swap");
        }
            ...
            if(IsClientInGame(iClient))
                if(GetClientTeam(iClient) == CS_TEAM_T) {
                    ...
                    if(IsPlayerAlive(iClient) && g_iTerroristsDeathCount) {
                        ...
                        PrintToChat(iClient, " \x04[\x02HNS\x04] \x06%t", "Points For T Surviving and Win", iPoints, g_iRoundPoints);
                    }
                    else
                        PrintToChat(iClient, " \x04[\x02HNS\x04] \x06%t", "Points For Win", g_iRoundPoints);
                }
        }
    }
    else if(iWinningTeam == CS_TEAM_CT)
    {
        ...
        SwapTeams();
        PrintToChatAll(" \x04[\x02HNS\x04] \x06%t", "CT Win");
        g_iTWinsInARow = 0;
...
stock void GiveGrenades(int iClient)
{
    ...
    if(iLastType == -1)
        PrintToChat(iClient, " \x04[\x02HNS\x04] \x06%t", "No Grenades");
    else {
        ...
        PrintToChat(iClient, " \x04[\x02HNS\x04] \x06%t", "Grenades Received", sGrenadeMessage);
    }
}
...
void Freeze(int iClient, float fDuration, int iType, int iAttacker = 0)
{
    ...
    if(iAttacker == 0) {
        PrintToChat(iClient, " \x04[\x02HNS\x04] \x06%t", "Frozen", fDuration);
    }
    else if(iAttacker == iClient) {
        PrintToChat(iClient, " \x04[\x02HNS\x04] \x06%t", "Frozen Yourself", fDuration);
    }
    else if(iAttacker <= MaxClients) {
        if(IsClientInGame(iAttacker)) {
            char sAttackerName[MAX_NAME_LENGTH];
            GetClientName(iAttacker, sAttackerName, sizeof(sAttackerName));
            PrintToChat(iClient, " \x04[\x02HNS\x04] \x06%t", "Frozen By", sAttackerName, fDuration);
        }
    }
    if(g_baFrozen[iClient]) {
        if(g_haFreezeTimer[iClient] != null) {
...
public Action Unfreeze(Handle hTimer, any iClient)
{
    if(iClient && IsClientInGame(iClient)) {
        if(g_baFrozen[iClient]) {
            ...
            if(IsPlayerAlive(iClient))
                PrintToChat(iClient, " \x04[\x02HNS\x04] \x06%t", "Unfreeze");
        }
    }
    return Plugin_Continue;
}
...
public Action UnfreezeCountdown(Handle hTimer, any iClient)
{
    if(iClient && IsClientInGame(iClient)) {
        ...
        if(IsPlayerAlive(iClient) && !g_bRespawnMode)
            PrintToChat(iClient, " \x04[\x02HNS\x04] \x06%t", "Round Start");
    }
    return Plugin_Continue;
}
...
{
    PrintToChat(iClient, " \x04[\x02HNS\x04] \x06%t", "Welcome Msg", PLUGIN_VERSION, AUTHOR, g_bRespawnMode ? "Respawn Mode" : "Normal Mode");
}
