...
{
	name		= "[CSGO] IP Adverts Blocker",
    author      = "R1KO | Edited: somebody.",
    description = "IP Adverts Blocker",
    version     = "1.0",
...
public OnPluginStart()
{
	...
	g_ImmFlag = CreateConVar("sm_ip_immuniti_flag", "z", "Admin flag for immunity (Blank - No immunity)");
...
{ 
	...
	if(CheckIP(text, sizeof(text)))
	{
	   ...
	   else PrintToChat(client, "[SM] IP hírdetés blokkolva. Ha még egyszer leírod, kitiltást kapsz!");
	   return Plugin_Handled;
	}
...
Client_Punishment(client)
{
	if(GetConVarBool(g_punishment_mode)) ServerCommand("sm_ban #%d %d Játékszerverek hírdetése", GetClientUserId(client), GetConVarInt(g_ban_time));
	else KickClient(client, "Játékszerverek hírdetése");
