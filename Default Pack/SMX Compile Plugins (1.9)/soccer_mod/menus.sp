...
public int MenuHandlerSoccer(Menu menu, MenuAction action, int client, int choice)
{
    if (action == MenuAction_Select)
    {
        ...
        if (StrEqual(menuItem, "admin"))
        {
            ...
            {
                PrintToChat(client, "[\x04Soccer Mod\x01] %t", "You are not allowed to use this option");
        ...
        {
            PrintToChat(client, "[\x04Soccer Mod\x01] %t", "Soccer Mod is not allowed on this map");
            OpenMenuSoccer(client);
        }
    }
...
public void OpenMenuSpecPlayer(int client)
{
    ...
    {
        PrintToChat(client, "[\x04Soccer Mod\x01] %t", "All players already are in spectator");
        OpenMenuAdmin(client);
    }
}
    ...
    if (action == MenuAction_Select)
    {
        ...
        if (IsClientInGame(target) && IsClientConnected(target))
        {
            if (GetClientTeam(target) != 1)
            {
                ...
                for (int player = 1; player <= MaxClients; player++)
                {
                    if (IsClientInGame(player) && IsClientConnected(player)) PrintToChat(player, "[\x04Soccer Mod\x01] %t", "$player has put $target to spectator", client, target);
                }
            ...
            else PrintToChat(client, "[\x04Soccer Mod\x01] %t", "Player is already in spectator");
        }
        else PrintToChat(client, "[\x04Soccer Mod\x01] %t", "Player is no longer on the server");
...
public int MenuHandlerMapsAdd(Menu menu, MenuAction action, int client, int choice)
{
    if (action == MenuAction_Select)
    {
        ...
        if (FindStringInArray(allowedMaps, map) > -1) PrintToChat(client, "[\x04Soccer Mod\x01] %t", "$map is already added to the allowed maps list", map);
        else
        {
            PushArrayString(allowedMaps, map);
            SaveAllowedMaps();

            PrintToChat(client, "[\x04Soccer Mod\x01] %t", "$map added to the allowed maps list", map);
        }
...
public void OpenMenuMapsChange(int client)
{
    ...
    {
        PrintToChat(client, "[\x04Soccer Mod\x01] %t", "Allowed maps list is empty");
        OpenMenuMaps(client);
    }
}
...
public int MenuHandlerMapsChange(Menu menu, MenuAction action, int client, int choice)
{
    if (action == MenuAction_Select)
    {
        ...
        for (int player = 1; player <= MaxClients; player++)
        {
            if (IsClientInGame(player) && IsClientConnected(player)) PrintToChat(player, "[\x04Soccer Mod\x01] %t", "$player has changed the map to $map", client, map);
        }
...
public int MenuHandlerMapsRemove(Menu menu, MenuAction action, int client, int choice)
{
    if (action == MenuAction_Select)
    {
        ...
        if (FindStringInArray(allowedMaps, map) > -1)
        {
            ...
            PrintToChat(client, "[\x04Soccer Mod\x01] %t", "$map removed from the allowed maps list", map);
        }
        else PrintToChat(client, "[\x04Soccer Mod\x01] %t", "$map is already removed from the allowed maps list", map);
...
public int MenuHandlerCommands(Menu menu, MenuAction action, int client, int choice)
{
    if (action == MenuAction_Select)
    {
        ...
        if (StrEqual(menuItem, "soccer"))           PrintToChat(client, "[\x04Soccer Mod\x01] %t", "Opens the Soccer Mod main menu");
        else if (StrEqual(menuItem, "help"))        PrintToChat(client, "[\x04Soccer Mod\x01] %t", "Opens the Soccer Mod help menu");
        else if (StrEqual(menuItem, "stats"))       PrintToChat(client, "[\x04Soccer Mod\x01] %t", "Opens the Soccer Mod statistics menu");
        else if (StrEqual(menuItem, "rank"))        PrintToChat(client, "[\x04Soccer Mod\x01] %t", "Shows your public ranking");
        else if (StrEqual(menuItem, "gk"))          PrintToChat(client, "[\x04Soccer Mod\x01] %t", "Enables or disables the goalkeeper skin");
        else if (StrEqual(menuItem, "pick"))        PrintToChat(client, "[\x04Soccer Mod\x01] %t", "Opens the Soccer Mod cap picking menu");
        else if (StrEqual(menuItem, "admin"))       PrintToChat(client, "[\x04Soccer Mod\x01] %t", "Opens the Soccer Mod admin menu");
        else if (StrEqual(menuItem, "cap"))         PrintToChat(client, "[\x04Soccer Mod\x01] %t", "Opens the Soccer Mod cap match menu");
        else if (StrEqual(menuItem, "match"))       PrintToChat(client, "[\x04Soccer Mod\x01] %t", "Opens the Soccer Mod match menu");
        else if (StrEqual(menuItem, "training"))    PrintToChat(client, "[\x04Soccer Mod\x01] %t", "Opens the Soccer Mod training menu");
        else if (StrEqual(menuItem, "restart"))     PrintToChat(client, "[\x04Soccer Mod\x01] %t", "Restarts the round");
        else if (StrEqual(menuItem, "commands"))    PrintToChat(client, "[\x04Soccer Mod\x01] %t", "Opens the Soccer Mod commands menu");
        else if (StrEqual(menuItem, "credits"))     PrintToChat(client, "[\x04Soccer Mod\x01] %t", "Opens the Soccer Mod credits menu");
...
public void OpenMenuCredits(int client)
{
    ...
    Format(langString, sizeof(langString), "%T", "Soccer Mod version", client);
