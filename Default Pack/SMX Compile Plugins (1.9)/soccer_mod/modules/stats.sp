    ...
    if (winner > 1)
    {
        if (statsScorerClientid)
        {
            ...
            if (statsScorerTeam == winner)
            {
                ...
                {
                    if (IsClientInGame(client) && IsClientConnected(client)) PrintToChat(client, "[\x04Soccer Mod\x01] %t", "Goal scored by $player", statsScorerName);
                }
                ...
                if (statsAssisterClientid > 0 && statsAssisterTeam == winner)
                {
                    ...
                    for (int client = 1; client <= MaxClients; client++)
                    {
                        if (IsClientInGame(client) && IsClientConnected(client)) PrintToChat(client, "[\x04Soccer Mod\x01] %t", "Assist given by $player", statsAssisterName);
                    }
            ...
            else
            {
                ...
                for (int client = 1; client <= MaxClients; client++)
                {
                    if (IsClientInGame(client) && IsClientConnected(client)) PrintToChat(client, "[\x04Soccer Mod\x01] %t", "Own goal scored by $player", statsScorerName);
                }
...
public void ShowMVP()
{
    ...
    if (highestScore)
    {
        ...
        for (int player = 1; player <= MaxClients; player++)
        {
            if (IsClientInGame(player) && IsClientConnected(player)) PrintToChat(player, "[\x04Soccer Mod\x01] %t", "$player was the MVP this round with $number points", roundMVP, highestScore);
        }
    ...
    else
    {
        for (int player = 1; player <= MaxClients; player++)
        {
            if (IsClientInGame(player) && IsClientConnected(player)) PrintToChat(player, "[\x04Soccer Mod\x01] %t", "MVP: No player scored more than 0 points this round");
        }
    }
}
...
public void ShowManOfTheMatch()
{
    ...
    if (highestScore)
    {
        ...
        for (int player = 1; player <= MaxClients; player++)
        {
            if (IsClientInGame(player) && IsClientConnected(player)) PrintToChat(player, "[\x04Soccer Mod\x01] %t", "The man of the match was $player with $number points", matchMVP, highestScore);
        }
    ...
    else
    {
        for (int player = 1; player <= MaxClients; player++)
        {
            if (IsClientInGame(player) && IsClientConnected(player)) PrintToChat(player, "[\x04Soccer Mod\x01] %t", "MOTM: No player scored more than 0 points this match");
        }
    }
}
