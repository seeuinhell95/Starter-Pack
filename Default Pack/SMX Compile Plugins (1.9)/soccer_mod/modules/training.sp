...
public void TrainingCannonSet(int client, char type[32], float number, float min, float max)
{
    if (number >= min && number <= max)
    {
        ...
        if (StrEqual(type, "randomness"))
        {
            ...
            {
                if (IsClientInGame(player) && IsClientConnected(player)) PrintToChat(player, "[\x04Soccer Mod\x01] %t", "$player has set the cannon randomness to $number", client, number);
            }
        ...
        else if (StrEqual(type, "fire_rate"))
        {
            ...
            for (int player = 1; player <= MaxClients; player++)
            {
                if (IsClientInGame(player) && IsClientConnected(player)) PrintToChat(player, "[\x04Soccer Mod\x01] %t", "$player has set the cannon fire rate to $number", client, number);
            }
        ...
        else if (StrEqual(type, "power"))
        {
            ...
            for (int player = 1; player <= MaxClients; player++)
            {
                if (IsClientInGame(player) && IsClientConnected(player)) PrintToChat(player, "[\x04Soccer Mod\x01] %t", "$player has set the cannon power to $number", client, number);
            }
    ...
    else PrintToChat(client, "[\x04Soccer Mod\x01] %t", "Type a value between $min and $max", min, max);
}
...
public int TrainingMenuHandler(Menu menu, MenuAction action, int client, int choice)
{
    if (action == MenuAction_Select)
    {
        ...
        {
            PrintToChat(client, "[\x04Soccer Mod\x01] %t", "You can not use this option during a match");
            OpenTrainingMenu(client);
        }
    }
...
public int TrainingChooseBallMenuHandler(Menu menu, MenuAction action, int client, int choice)
{
    if (action == MenuAction_Select)
    {
        ...
        for (int player = 1; player <= MaxClients; player++)
        {
            if (IsClientInGame(player) && IsClientConnected(player)) PrintToChat(player, "[\x04Soccer Mod\x01] %t", "$player has turned the cannon on", client);
        }
...
public int TrainingCannonSettingsMenuHandler(Menu menu, MenuAction action, int client, int choice)
{
    if (action == MenuAction_Select)
    {
        ...
        {
            PrintToChat(client, "[\x04Soccer Mod\x01] %t", "Type a value between $min and $max", 0.0, 500.0);
            changeSetting[client] = "randomness";
        }
        else if (StrEqual(menuItem, "fire_rate"))
        {
            PrintToChat(client, "[\x04Soccer Mod\x01] %t", "Type a value between $min and $max", 0.5, 10.0);
            changeSetting[client] = "fire_rate";
        }
        else if (StrEqual(menuItem, "power"))
        {
            PrintToChat(client, "[\x04Soccer Mod\x01] %t", "Type a value between $min and $max", 0.001, 10000.0);
...
public Action TrainingCannonShoot(Handle timer)
{
    ...
    else
    {
        ...
        for (int player = 1; player <= MaxClients; player++)
        {
            if (IsClientInGame(player) && IsClientConnected(player)) PrintToChat(player, "[\x04Soccer Mod\x01] %t", "Ball cannon entity is invalid");
        }
    }
}
...
public void TrainingDisableGoals(int client)
{
    if (trainingGoalsEnabled)
    {
        ...
        for (int player = 1; player <= MaxClients; player++)
        {
            if (IsClientInGame(player) && IsClientConnected(player)) PrintToChat(player, "[\x04Soccer Mod\x01] %t", "$player has disabled the goals", client);
        }
    ...
    else PrintToChat(client, "[\x04Soccer Mod\x01] %t", "Goals are already disabled");
}
...
public void TrainingEnableGoals(int client)
{
    if (!trainingGoalsEnabled)
    {
        ...
        for (int player = 1; player <= MaxClients; player++)
        {
            if (IsClientInGame(player) && IsClientConnected(player)) PrintToChat(player, "[\x04Soccer Mod\x01] %t", "$player has enabled the goals", client);
        }
    ...
    else PrintToChat(client, "[\x04Soccer Mod\x01] %t", "Goals are already enabled");
}
...
public void TrainingSpawnBall(int client)
{
    ...
    else PrintToChat(client, "[\x04Soccer Mod\x01] %t", "Cant spawn model $model", trainingModelBall);
}
...
public void TrainingCannonOn(int client)
{
    if (trainingCannonTimer == null)
    {
        ...
        if (!IsValidEntity(trainingCannonBallIndex))
        {
            ...
            if (count > 1)
            {
                PrintToChat(client, "[\x04Soccer Mod\x01] %t", "More than one possible ball found");
    ...
    {
        PrintToChat(client, "[\x04Soccer Mod\x01] %t", "Cannon is already on");
        OpenTrainingCannonMenu(client);
    }
}
...
public void TrainingCannonOff(int client)
{
    if (trainingCannonTimer != null)
    {
        ...
        for (int player = 1; player <= MaxClients; player++)
        {
            if (IsClientInGame(player) && IsClientConnected(player)) PrintToChat(player, "[\x04Soccer Mod\x01] %t", "$player has turned the cannon off", client);
        }
    ...
    else PrintToChat(client, "[\x04Soccer Mod\x01] %t", "Cannon is already off");
}
...
public void TrainingCannonPosition(int client)
{
    ...
    for (int player = 1; player <= MaxClients; player++)
    {
        if (IsClientInGame(player) && IsClientConnected(player)) PrintToChat(player, "[\x04Soccer Mod\x01] %t", "$player has set the cannon position", client);
    }
...
public void TrainingCannonAimPosition(int client)
{
    ...
    for (int player = 1; player <= MaxClients; player++)
    {
        if (IsClientInGame(player) && IsClientConnected(player)) PrintToChat(player, "[\x04Soccer Mod\x01] %t", "$player has set the cannon aim position", client);
    }
