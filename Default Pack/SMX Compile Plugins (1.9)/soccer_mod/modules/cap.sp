...
public int CapMenuHandler(Menu menu, MenuAction action, int client, int choice)
{
    if (action == MenuAction_Select)
    {
        ...
        else PrintToChat(client, "[\x04Soccer Mod\x01] %t", "You can not use this option during a match");
    ...
    if (client)
    {
        if (client == capT || client == capCT)
        {
            if (client == capPicker)
            {
                ...
                else PrintToChat(client, "[\x04Soccer Mod\x01] %t", "No players available to pick");
            }
            else PrintToChat(client, "[\x04Soccer Mod\x01] %t", "It is not your turn to pick");
        }
        else PrintToChat(client, "[\x04Soccer Mod\x01] %t", "You are not a cap");
    }
}
    ...
    if (action == MenuAction_Select)
    {
        ...
        if (IsClientInGame(target) && IsClientConnected(target))
        {
            ...
            if (client == capCT)
            {
                ...
                for (int player = 1; player <= MaxClients; player++)
                {
                    if (IsClientInGame(player) && IsClientConnected(player)) PrintToChat(player, "[\x04Soccer Mod\x01] %t", "$player has picked $target", client, target);
                }
        ...
        {
            PrintToChat(client, "[\x04Soccer Mod\x01] %t", "Player is no longer on the server");
...
public Action TimerCapFightCountDown(Handle timer, any seconds)
{
    for (int player = 1; player <= MaxClients; player++)
    {
        if (IsClientInGame(player) && IsClientConnected(player)) PrintCenterText(player, "[\x04Soccer Mod\x01] %t", "Cap fight will start in $number seconds", seconds);
    }
}
...
public Action TimerCapFightCountDownEnd(Handle timer)
{
    for (int player = 1; player <= MaxClients; player++)
    {
        ...
        {
            PrintCenterText(player, "[\x04Soccer Mod\x01] %t", "FIGHT!");
...
public void CapPutAllToSpec(int client)
{
    for (int player = 1; player <= MaxClients; player++)
    {
        if (IsClientInGame(player) && IsClientConnected(player))
        {
            PrintToChat(player, "[\x04Soccer Mod\x01] %t", "$player has put all players to spectator", client);
...
public void CapAddRandomPlayer(int client)
{
    ...
    if (count)
    {
        ...
        for (int player = 1; player <= MaxClients; player++)
        {
            if (IsClientInGame(player) && IsClientConnected(player)) PrintToChat(player, "[\x04Soccer Mod\x01] %t", "$player has forced $target as random player", client, randomPlayer);
        }
    ...
    else PrintToChat(client, "[\x04Soccer Mod\x01] %t", "No players in spectator");
}
...
public void CapStartFight(int client)
{
    if (!capFightStarted)
    {
            ...
            if (IsClientInGame(player) && IsClientConnected(player))
            {
                ...
                else
                {
                    ...
                    if (!gk && !lb && !rb && !mf && !lw && !rw)
                    {
                        ...
                        PrintToChat(client, "[\x04Soccer Mod\x01] %t", "Please set your position to help the caps with picking");
                    }
                }
            }
        }
        ...
        for (int player = 1; player <= MaxClients; player++)
        {
            if (IsClientInGame(player) && IsClientConnected(player)) PrintToChat(player, "[\x04Soccer Mod\x01] %t", "$player has started a cap fight", client);
        }
    ...
    else PrintToChat(client, "[\x04Soccer Mod\x01] %t", "Cap fight already started");
}
