...
public void ClientCommandPublicRankingCallback(Handle owner, Handle hndl, const char[] error, any client)
{
    ...
    if (hndl == INVALID_HANDLE)
    {
        ...
        PrintToChat(client, "[\x04Soccer Mod\x01] %t", "You are not ranked yet");
    }
    else if (total)
    {
        ...
        {
            if (IsClientInGame(player) && IsClientConnected(player)) PrintToChat(player, "[\x04Soccer Mod\x01] %t", "$player is ranked $rank with $number points", client, rank, total, points);
        }
    }
    else PrintToChat(client, "[\x04Soccer Mod\x01] %t", "You are not ranked yet");
}
...
public void OpenRankingTopMenuCallback(Handle owner, Handle hndl, const char[] error, any pack)
{
    ...
    {
        LogError("Failed to query (error: %s)", error);
        PrintToChat(client, "[\x04Soccer Mod\x01] %t", "No players are ranked yet");
...
public void OpenRankingLastConnectedMenuCallback(Handle owner, Handle hndl, const char[] error, int client)
{
    ...
    {
        LogError("Failed to query (error: %s)", error);
        PrintToChat(client, "[\x04Soccer Mod\x01] %t", "No players found");
        OpenRankingMenu(client);
    }
    else if (SQL_GetRowCount(hndl))
    {
