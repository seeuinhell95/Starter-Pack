    ...
    if (matchStarted)
    {
        ...
        if (team > 1)
        {
            if (GetTeamClientCount(team) > matchMaxPlayers)
            {
                ...
                {
                    if (IsClientInGame(player) && IsClientConnected(player)) PrintToChat(player, "[\x04Soccer Mod\x01] %t", 
...
public int MatchScoreMenuHandler(Menu menu, MenuAction action, int client, int choice)
{
    if (action == MenuAction_Select)
    {
        ...
        if (StrEqual(menuItem, "add_ct"))
        {
            ...
            for (int player = 1; player <= MaxClients; player++)
            {
                if (IsClientInGame(player) && IsClientConnected(player)) PrintToChat(player, "[\x04Soccer Mod\x01] %t", "$player has added a goal to $team", client, "counter-terrorists");
            }
        ...
        else if (StrEqual(menuItem, "remove_ct"))
        {
            if (matchScoreCT > 0)
            {
                ...
                for (int player = 1; player <= MaxClients; player++)
                {
                    if (IsClientInGame(player) && IsClientConnected(player)) PrintToChat(player, "[\x04Soccer Mod\x01] %t", "$player has removed a goal from $team", client, "counter-terrorists");
                }
            ...
            else PrintToChat(client, "[\x04Soccer Mod\x01] %t", "Score is already 0");
        ...
        else if (StrEqual(menuItem, "add_t"))
        {
            ...
            for (int player = 1; player <= MaxClients; player++)
            {
                if (IsClientInGame(player) && IsClientConnected(player)) PrintToChat(player, "[\x04Soccer Mod\x01] %t", "$player has added a goal to $team", client, "terrorists");
            }
        ...
        else if (StrEqual(menuItem, "remove_t"))
        {
            if (matchScoreT > 0)
            {
                ...
                for (int player = 1; player <= MaxClients; player++)
                {
                    if (IsClientInGame(player) && IsClientConnected(player)) PrintToChat(player, "[\x04Soccer Mod\x01] %t", "$player has removed a goal from $team", client, "terrorists");
                }
        ...
        else if (StrEqual(menuItem, "reset"))
        {
            ...
            for (int player = 1; player <= MaxClients; player++)
            {
                if (IsClientInGame(player) && IsClientConnected(player)) PrintToChat(player, "[\x04Soccer Mod\x01] %t", "$player has reset the score", client);
            }
...
public Action DelayMatchEnd(Handle timer)
{
    ...
    for (int player = 1; player <= MaxClients; player++)
    {
        if (IsClientInGame(player) && IsClientConnected(player))
        {
            ...
            PrintToChat(player, "[\x04Soccer Mod\x01] %s: CT %i - %i T", langString, matchScoreCT, matchScoreT);
        }
    }
...
public void MatchStart(int client)
{
    if (!matchStarted)
    {
        ...
        for (int player = 1; player <= MaxClients; player++)
        {
            if (IsClientInGame(player) && IsClientConnected(player)) PrintToChat(player, "[\x04Soccer Mod\x01] %t", "$player has started a match", client);
        }
    ...
    else PrintToChat(client, "[\x04Soccer Mod\x01] %t", "Match already started");
}
...
public void MatchPause(int client)
{
    if (matchStarted)
    {
        if (!matchPaused)
        {
            ...
            for (int player = 1; player <= MaxClients; player++)
            {
                if (IsClientInGame(player) && IsClientConnected(player)) PrintToChat(player, "[\x04Soccer Mod\x01] %t", "$player has paused the match", client);
            }
        ...
        else PrintToChat(client, "[\x04Soccer Mod\x01] %t", "Match already paused");
    }
    else PrintToChat(client, "[\x04Soccer Mod\x01] %t", "No match started");
}
...
public void MatchUnpause(int client)
{
    if (matchStarted)
    {
        if (matchPaused)
        {
            ...
            for (int player = 1; player <= MaxClients; player++)
            {
                if (IsClientInGame(player) && IsClientConnected(player)) PrintToChat(player, "[\x04Soccer Mod\x01] %t", "$player has unpaused the match", client);
            }
        ...
        else PrintToChat(client, "[\x04Soccer Mod\x01] %t", "Match already unpaused");
    }
    else PrintToChat(client, "[\x04Soccer Mod\x01] %t", "No match started");
}
...
public void MatchStop(int client)
{
    if (matchStarted)
    {
        ...
        for (int player = 1; player <= MaxClients; player++)
        {
            if (IsClientInGame(player) && IsClientConnected(player)) PrintToChat(player, "[\x04Soccer Mod\x01] %t", "$player has stopped the match", client);
        }
    ...
    if (matchStoppageTimeStarted)
    {
        ...
        else
        {
            if (matchGoldenGoal && matchScoreCT == matchScoreT)
            {
                ...
                for (int player = 1; player <= MaxClients; player++)
                {
                    if (IsClientInGame(player) && IsClientConnected(player)) PrintToChat(player, "[\x04Soccer Mod\x01] %t", "The match ended in a draw and will continue with a golden goal");
                }
