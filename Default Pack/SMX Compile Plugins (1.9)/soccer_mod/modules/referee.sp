...
public void RefereeEventPlayerSpawn(Event event)
{
    ...
    if (PlayerHasCard(clientSteamid, "red"))
    {
        ...
        PrintToChat(client, "[\x04Soccer Mod\x01] %t", "You have been put to spectator because you have a red card");
    }
}
...
public void OpenRefereeYellowCardMenu(int client)
{
    ...
    {
        PrintToChat(client, "[\x04Soccer Mod\x01] %t", "All players already have a red card");
    ...
    if (action == MenuAction_Select)
    {
        ...
        if (IsClientInGame(target) && IsClientConnected(target))
        {
            ...
            if (keygroup.GetNum("yellow", 0))
            {
                ...
                for (int player = 1; player <= MaxClients; player++)
                {
                    if (IsClientInGame(player) && IsClientConnected(player)) PrintToChat(player, "[\x04Soccer Mod\x01] %t", "$player has given a second yellow card to $target", client, target);
                }
            ...
            else
            {
                ...
                for (int player = 1; player <= MaxClients; player++)
                {
                    if (IsClientInGame(player) && IsClientConnected(player)) PrintToChat(player, "[\x04Soccer Mod\x01] %t", "$player has given a yellow card to $target", client, target);
                }
    ...
    if (action == MenuAction_Select)
    {
        ...
        if (IsClientInGame(target) && IsClientConnected(target))
        {
            ...
            if (!keygroup.GetNum("red", 0))
            {
                ...
                for (int player = 1; player <= MaxClients; player++)
                {
                    if (IsClientInGame(player) && IsClientConnected(player)) PrintToChat(player, "[\x04Soccer Mod\x01] %t", "$player has given a red card to $target", client, target);
                }

                LogMessage("%N <%s> has given a red card to %N <%s>", client, clientSteamid, target, targetSteamid);
            }
            else PrintToChat(client, "[\x04Soccer Mod\x01] %t", "Player already has a red card");
...
public void OpenRemoveRedCardMenu(int client)
{
    ...
    {
        PrintToChat(client, "[\x04Soccer Mod\x01] %t", "There are no players with a red card");
        OpenRefereeMenu(client);
    }
}
...
public int RemoveRedCardMenuHandler(Menu menu, MenuAction action, int client, int choice)
{
    if (action == MenuAction_Select)
    {
        ...
        if (keygroup.GetNum("red", 0))
        {
            ...
            for (int player = 1; player <= MaxClients; player++)
            {
                if (IsClientInGame(player) && IsClientConnected(player)) PrintToChat(player, "[\x04Soccer Mod\x01] %t", "$player has removed the red card from $target", client, playerName);
            }
        ...
        else PrintToChat(client, "[\x04Soccer Mod\x01] %t", "Red card already removed");
...
public void RemoveAllCards(int client)
{
    ...
    for (int player = 1; player <= MaxClients; player++)
    {
        if (IsClientInGame(player) && IsClientConnected(player)) PrintToChat(player, "[\x04Soccer Mod\x01] %t", "$player has removed all cards", client);
    }
